...
class DatabaseHelper {
  ...
  Future _onCreate(Database db, int version) async {
    ...
    await db.rawInsert(
        '            INSERT INTO User (UserFirstName,UserLastName,UserPassword,UserPhoneNumber,UserDOB,CenterID,UserGender) values("Murat", "Kara", "mstf123", "05352551325", "1998-02-15", 1, "M")');
    await db.rawInsert(
        '            INSERT INTO User (UserFirstName,UserLastName,UserPassword,UserPhoneNumber,UserDOB,CenterID,UserGender) values("Ege", "Deniz", "mstf123", "05352551325", "1998-02-15", 1, "M")');
  ...
  DeleteItem(int itemID) async{
    Database? db = await instance.database;
    var list=await db!.rawQuery('DELETE from WorkoutPlan where ItemID=$itemID');
  }
  DeleteAccount(int id) async{
    Database? db = await instance.database;

    var list2=await db!.rawQuery('DELETE from WorkoutPlan where UserID=$id');
    var list=await db!.rawQuery('DELETE from User where UserID=$id');
  ...
  Future<int> delete(int id,String tableName) async {
    Database? db = await instance.database;
    var dataId=await db!.query(tableName,where: '$columnMovieID = ?',whereArgs: [id]);
    await db!.delete(tableName, where: '$columnMovieID = ?', whereArgs: [id]);
    return dataId[0][columnId];
  }
